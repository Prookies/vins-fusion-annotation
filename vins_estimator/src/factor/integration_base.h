...
public:
  ...
  void repropagate(const Eigen::Vector3d &_linearized_ba,
    ...
    sum_dt = 0.0;
    // 初始化加速度和角速度
    // linearized_acc是两帧之间IMU数据初始的加速度
    // linearized_gyr是两帧之间IMU数据初始的角速度
  ...
  */
  ...
  void propagate(double _dt, const Eigen::Vector3d &_acc_1,
    ...
    // 中值积分
    // 在初始化之后，delta_p,delta_q,delta并没有进行其他赋值
    // 而初始化时，delta_p和delta_v为0向量，delta_q为单位四元数
    // acc_0,gyr_0是上一次的imu数据
