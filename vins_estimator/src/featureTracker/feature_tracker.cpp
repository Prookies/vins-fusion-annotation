...
/**
 * @brief FeatureTracker::distance
 *        判断两个特征点之间的距离，使用欧式几何距离
 * @param pt1
 * @param pt2
 * @return
 */
...
// NOTE:能否提取了左右两幅图像的特征点，
// 然后匹配完之后直接计算特征点的相对于该相机的3维位置
  ...
  */
  ...
  if (prev_pts.size() > 0) {
    ...
    if (hasPrediction) {
      ...
      // 预测的特征点
      // NOTE: predict_pts怎么得到的
      cur_pts = predict_pts;
      // 计算光流
      cv::calcOpticalFlowPyrLK(
          prev_img, cur_img, prev_pts, cur_pts, status, err, cv::Size(21, 21),
          1, cv::TermCriteria(cv::TermCriteria::COUNT + cv::TermCriteria::EPS,
                              30, 0.01),
    ...
    if (FLOW_BACK) {
      ...
      // 遍历所有特征点的状态
      for (size_t i = 0; i < status.size(); i++) {
        // 好的特征点的要求：正向跟踪成功，反向跟踪成功，并且前一帧特征点和反向跟踪得到的前一帧的特征点距离较近
  ...
  if (1) {
    ...
    TicToc t_m;
    // 绘制掩模图像，保证特征点之间距离为30个像素
    setMask();
    ROS_DEBUG("set mask costs %fms", t_m.toc());

    ROS_DEBUG("detect feature begins");
    TicToc t_t;
    // 补充新的特征点，并且保证不超过最大的特征点数量
  ...
  if (!_img1.empty() && stereo_cam) {
    ...
    if (!cur_pts.empty()) {
      ...
      // cur left ---- cur right
      // 使用光流法计算右图与左图对应的特征点
      // 可以加入RANSAC算法或者基于已知的左右相机相对变换R,t构建对极约束去除外点
      ...
      if (FLOW_BACK) {
        cv::calcOpticalFlowPyrLK(rightImg, cur_img, cur_right_pts,
                                 ...
                                 cv::Size(21, 21), 3);
        // 好的特征点的要求：正反两次光流法都能成功跟踪，在视野范围内，距离相近
        for (size_t i = 0; i < status.size(); i++) {
  ...
  // 如果是双目模式，则把右目的特征也存储到featureFrame中
  // 对与右目，其camera_id设置为1
  if (!_img1.empty() && stereo_cam) {
...
/**
 * @brief FeatureTracker::ptsVelocity
 *        计算角点的速度
 * @param ids 表示特征点的编号
 * @param pts 当前特征点
 * @param cur_id_pts <int,cv::Point2f> 特征点编号及坐标
 * @param prev_id_pts <int,cv::Point2f> 先前特征点编号及坐标
 * @return
 */
vector<cv::Point2f>
