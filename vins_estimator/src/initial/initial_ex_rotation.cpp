...
/**
 * @brief InitialEXRotation::CalibrationExRotation
 *        标定相机外参
 * @param corres
 * @param delta_q_imu
...
bool InitialEXRotation::CalibrationExRotation(
  ...
  // ric是参数文件中的IMU与相机之间的旋转矩阵，其初始化为单位矩阵
  // Rc_g表示每帧IMU相对于起始帧IMU的旋转矩阵
  Rc_g.push_back(ric.inverse() * delta_q_imu * ric);

  // Ax = b
  // NOTE:这样求解的精度可能会比较低，考虑两个情况：
  // 矩阵中是否会有较大的数值
  // 可以采用OpenCV中求解基础矩阵的方法
...
/**
 * @brief InitialEXRotation::solveRelativeR
 *        根据两帧特征点求解两帧之间的相对旋转
 * @param corres
 * @return
 */
Matrix3d InitialEXRotation::solveRelativeR(
  ...
  if (corres.size() >= 9) {
    ...
    // TODO: 这里用的是二维的归一化平面的点，如果能获得3维点是不是可以简单很多
    // 求解两帧的基础矩阵
